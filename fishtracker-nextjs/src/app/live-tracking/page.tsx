...
const LiveTrackingPage: React.FC = () => {
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      setCameraError(
        "Camera access denied. Please enable camera permissions or upload an image from your gallery."
      );
  ...
  const startLiveFeed = () => {
    ...
    showToast(
      "Live feed started - Auto-capturing every 5 seconds",
      "success",
      4000
    );
  ...
  const stopLiveFeed = () => {
    ...
    setStatusMessage(
      `Live feed stopped - ${captureCount} captures, ${totalFishDetected} fish detected`
    );
    showToast(
      `Live feed stopped: ${captureCount} captures, ${totalFishDetected} fish found`,
      "info",
      5000
    );
  };
  ...
  const autoCaptureAndUpload = useCallback(async () => {
    if (isProcessing) {
      console.log(
        "⏭️ [TRACKING] Skipping capture - still processing previous image"
      );
    ...
    try {
      ...
      if (ctx) {
        ...
        console.log(
          `📸 [TRACKING] Image captured: ${canvas.width}x${canvas.height}`
        );
        ...
        console.log(
          `📤 [TRACKING] Auto-uploading capture #${currentCapture}...`
        );
        ...
        const response = await fishTrackerApi.uploadFishImage(
          ...
          (progress) => {
            console.log(
              `📊 [TRACKING] Upload #${currentCapture}: ${progress.toFixed(0)}%`
            );
          }
        );
        ...
        if (response.success && response.data.fishDetected) {
          ...
          setDetectedFish((prev) =>
            [...response.data.fishes, ...prev].slice(0, 10)
          ); // Keep last 10
          setLatestFish(response.data.fishes[0]);

          console.log(
            `✅ [TRACKING] Capture #${currentCapture}: ${fishCount} fish detected!`
          );
          addDebugLog(
            "success",
            `Auto-capture #${currentCapture} - ${fishCount} fish detected`,
            {
              fishes: response.data.fishes.map((f) => ({
                name: f.name,
                accuracy: f.aiAccuracy,
              })),
            }
          );

          showToast(
            `🐟 Found ${fishCount} fish! (Total: ${
              totalFishDetected + fishCount
            })`,
            "success",
            3000
          );
          setStatusMessage(
            `🔴 LIVE: ${fishCount} fish found in capture #${currentCapture}`
          );
        } else {
          console.log(
            `ℹ️ [TRACKING] Capture #${currentCapture}: No fish detected`
          );
          addDebugLog(
            "info",
            `Auto-capture #${currentCapture} - No fish detected`
          );
      ...
      console.error(
        `❌ [TRACKING] Auto-capture #${currentCapture} failed:`,
        err
      );
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      console.log(
        `📁 [TRACKING] User selected file: ${file.name} (${(
          file.size /
          1024 /
          1024
        ).toFixed(2)}MB)`
      ...
      if (file.size > 5 * 1024 * 1024) {
        ...
        addDebugLog("error", "File size exceeds 5MB limit", {
          size: file.size,
        });
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      if (response.success && response.data.fishDetected) {
        ...
        console.log(
          `✅ [TRACKING] Upload successful - ${response.data.fishes.length} fish found`
        );
  ...
  const viewFishDetails = (fish: Fish) => {
    ...
    addDebugLog("info", "User viewing fish details", {
      fishName: fish.name,
      fishId: fish.id,
    });
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black to-transparent">
                  <div className="flex items-center justify-center gap-4">
                    ...
                    <button
                      ...
                      aria-label={
                        liveFeedMode ? "Stop live feed" : "Start live feed"
                      }
                    >
                      {liveFeedMode ? (
                        <svg
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex gap-2 mt-2 flex-wrap">
                        ...
                        <Badge
                          variant={fish.aiAccuracy > 80 ? "success" : "warning"}
                        >
        ...
        >
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium">
                  {selectedFish.minSize} - {selectedFish.maxSize} cm
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Depth Range</p>
                <p className="font-medium">
                  {selectedFish.depthRangeMin} - {selectedFish.depthRangeMax} m
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              <Badge
                variant={
                  selectedFish.conservationStatus === "Least Concern"
                    ? "success"
                    : selectedFish.conservationStatus === "Endangered" ||
            ...
            <div className="flex gap-3 pt-4">
              <Button
                onClick={() => viewFishDetails(selectedFish)}
                variant="primary"
                className="flex-1"
              >
                View Full Details
